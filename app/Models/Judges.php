...
use App\Filament\Resources\JudgesResource\RelationManagers\ProceedingRelationManagerRelationManager;
...
class Judges extends Model
{
    ...
    {
        // e’tibor: sizning FK — inspection_cases_id (plural). Shuni belongsTo'ga yozing!
    ...
    public function familiesParents7()
    {
        return $this->hasMany(\App\Models\Family::class, 'judge_id', 'id')
            ->where('parents_id', 7); // faqat parents_id = 7
    }
    ...
    {
        return $this->hasMany(JudgeActivity::class, 'judge_id');
    }
...
//    }
    ...
    {
        return trim("{$this->middle_name} {$this->first_name} {$this->last_name}");
    }

    public function getFirstStageAttribute()
    {
        return $this->judges_stages->sortBy('start_date')->first()?->position ?? '—';
    }
    ...
    public function getImageUrlAttribute(): ?string
    {
        return $this->image
            ...
            : null; // ❗ null guard, aks holda Storage::url(null) bo‘ladi
    }


    public function judge_activity_entries()
    {
        return $this->hasOne(JudgeActivityEntry::class, 'judge_id');
    }

    public function getRatingAttribute(): float
    {
        // adding_rating ga qo‘shamiz (faqat entry bor bo‘lsa)
        $adding = (float)($this->adding_rating ?? 0);
        if ($this->judge_activity_entries()->exists()) {
            $adding += 5.0;
        }

        return
            (float)($this->quality_score ?? 0)
            + (float)($this->etiquette_score ?? 0)
            + (float)($this->ethics_score ?? 0)
            + (float)($this->foreign_language_bonus ?? 0)
            + (float)($this->foreign_language_bonus ?? 0)
            + $adding;
    }
    ...
    {
        return $this->hasMany(Proceeding::class, 'judge_id');
    }
    public function penaltiesApplied(){ return $this->hasMany(\App\Models\AppliedPenalty::class, 'judge_id'); }
