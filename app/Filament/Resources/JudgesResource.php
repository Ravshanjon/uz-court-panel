                        ...
                        Tab::make('Судья маълумотлари')->schema([
                            Grid::make(4)->schema([
                                ...
                                TextInput::make('age')
                                    ->label('Ёши')
                                    ->numeric()
                                    ->readOnly() // foydalanuvchi o‘zgartirmasin
                                    ->dehydrated(true) // saqlanishi uchun shart
                                    ->default(0),
...
//                                ->content(fn($record) => view('components.judges-stages-tab', ['record' => $record])),
                        ...
                        Tab::make('Оилавий Ҳолати')->schema([
                            Placeholder::make('family')
                                ->label('')
                                ->content(fn($record) => view('components.family', ['record' => $record])),
                        ])
                                ...
                                ->tabs([
                                    Tab::make('Сифати')
                                        ...
                                        ->badge(function ($record) {
                                            // badge: aktiv penaltylar soni
                                            return AppliedPenalty::where('judge_id', $record->id)->active()->count();
                                        })
                                    ...
                                    Tab::make('Фаоллиги')
                                        ->schema([
                                            Placeholder::make('bonus')
                                                ->label('')
                                                ->content(fn($record) => view('components.judges-activity-input', ['record' => $record])),
                                        ]),

                                    Tab::make('bonus-tab')
                                        ->label(fn($record) => 'Қўшимча баллар')
                                        ->badge(fn($record) => $record->bonuses->count())
                                        ->badgeColor('warning')
...
//                    }),
                ...
                TextColumn::make('judges_stages')
                    ->label('Ваколати')
                    ->html()
                    ->getStateUsing(function ($record) {
                        return $record->judges_stages->map(function ($stage) {
                            $from = \Carbon\Carbon::parse($stage->start_date)->format('d.m.Y');
                            $to = \Carbon\Carbon::parse($stage->end_date)->format('d.m.Y');
                            return "дан: {$from}<br>гача: {$to}";
                        })->implode('<br><br>');
                    }),
                ...
                Tables\Columns\TextColumn::make('birth_date')
                    ...
                    ->getStateUsing(function ($record) {
                        ...
                        $date = \Carbon\Carbon::parse($record->birth_date);
                        $formattedDate = $date->format('d.m.Y');

                        // 🎂 faqat tug‘ilgan kun bugungi kunga to‘g‘ri kelsa chiqadi
                        $icon = $date->isBirthday() ? ' 🎁' : '';

                        $regionName = $record->region ? $record->region->name : '';

                        return "{$formattedDate}{$icon} <br> <span class='text-gray-500 text-sm'>{$regionName}</span>";
                    })
                    ->html()
                    ...
                    Section::make()->schema([
                        ...
                        Infolists\Components\Tabs::make()->schema([
                            ...
                            Infolists\Components\Tabs\Tab::make('Оилавий ҳолати')->schema([
                                ...
                                ViewEntry::make('family')
                                    ->view('components.family', fn($record) => [
